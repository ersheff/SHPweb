...
export default class HUB {
    ...
    verboseServerMessage: boolean;
    options: CHConstructorOptions;
    /**
    ...
    constructor(options: CHConstructorOptions) {
        ...
        this.Users = new Map<string, CHUserDetails>();
        this.Controls = [];
        this.Events = [];
        this.Rooms = [];
    ...
    onNewConnection(socket: SocketIO.Socket) {
        // executed when a new connect is made
        const snsp: string = socket.nsp.name;
        const _sock = socket;
        ...
        socket.on('chat', (data: CHChatToServer) => {
            chat(data);
        });

        socket.on('addUsername', (data: CHUsername) => {
            addUsername(data);
        });

        socket.on('event', (data: CHEventToServer) => {
            event(data);
        });
        ...
        socket.on('clearControl', (data: CHClearControls) => {
            clearControls(data);
        });
        ...
        socket.on('clearEvent', (data: CHClearEvents) => {
            clearEvents(data);
        });
        ...
        socket.on('control', (data: CHControlToServer) => {
            control(data);
        });

        socket.on('observeControl', (data: CHObserve) => {
            observeControl(data, observedControls);
        });

        socket.on('unobserveControl', (data: CHObserve) => {
        ...
        socket.on('observeAllControl', (data: CHObserveAll) => {
            observeAllControl(data);
        });

        socket.on('observeEvent', (data: CHObserve) => {
            observeEvent(data, observedEvents);
        });

        socket.on('unobserveEvent', (data: CHObserve) => {
            unobserveEvent(data);
        });
        ...
        socket.on('observeAllEvents', (data: CHObserveAll) => {
            observeAllEvents(data);
        });

        socket.on('getControl', (data: CHObserve) => {
            getControl(data);
        });

        socket.on('setConsoleDisplay', (boolState: CHBoolState) => {
        ...
        socket.on('joinRoom', (data: CHRoom) => {
            joinRoom(data);
        });

        socket.on('leaveRoom', (room: CHRoom) => {
            leaveRoom(room);
        });
        ...
        const getControl = (data: CHObserve) => {
            ...
            if (data.header === 'dump') {
                ...
                const _controlObject: CHControlToClient = {
        ...
        const event = (data: CHEventToServer) => {
            ...
            if (data.mode === Mode.PUBLISH) {
                ...
                if (_event === null || _event === undefined) {
                    ...
                    const eventObject: CHEventToClient = {
                        header: data.header,
                    ...
                    const allEventObservers = Array.from(this.Users.values()).filter((user: CHUserDetails) => user.observeallevents === true).map((user: CHUserDetails) => user.id);
        ...
        const control = (data: CHControlToServer) => {
            ...
            if (data.mode === Mode.PUBLISH) {
                ...
                if (_control === null || _control === undefined) {
                    ...
                    const _controlObject: CHControl = {
                        header: data.header,
                        values: data.values,
                        from: socket.id,
                        observers: [],
                ...
                const controlObject: CHControlToClient = {
                    header: data.header,
                    values: data.values,
                ...
                const allControlObservers = Array.from(this.Users.values()).filter((user: CHUserDetails) => user.observeallcontrol === true).map((user: CHUserDetails) => user.id);
        ...
        const chat = (data: CHChatToServer) => {
            ...
            const _chatObject: CHChatToClient = {
        ...
        const broadcast = (type: CHMessageType, targets: string[] | string, dataObject: object) => {
            switch (type) {
        ...
        const myUsername = (whichSocket?: string) => {
        ...
        const changeUserProperty = (property, value) => {
            ...
            if (this.Users.has(socket.id)) {
                const _userObject: CHUserDetails = this.Users.get(socket.id);
        ...
        const observeControl = (data: CHObserve, callback?) => {
            if (!data.hasOwnProperty('header')) {
                // error
        ...
        const unobserveControl = (data: CHObserve) => {
        ...
        const myEvents = (whichSocket?: string) => {
            if (whichSocket === undefined) {
                whichSocket = socket.id;
            }

            const tempUsers: Map<string, CHUserDetails> = _.cloneDeep(this.Users);
            if (!tempUsers.has(whichSocket)) {
                return [];
            }

            const tempEvents: CHControl[] = _.cloneDeep(this.Events);
            const _myEvents = tempUsers.get(whichSocket).myEvents;
            const foundEvents: CHControl[] = tempEvents.filter(target => _myEvents.includes(target.header));
        ...
        const myControls = (whichSocket?: string) => {
            if (whichSocket === undefined) {
                whichSocket = socket.id;
            }
            ...
            const tempControls: CHControl[] = _.cloneDeep(this.Controls);
            const _myControls = tempUsers.get(whichSocket).myControls;
            const foundControls: CHControl[] = tempControls.filter(target => _myControls.includes(target.header));
        ...
        const unpublishControl = (data: CHObserve, callback?) => {
            if (!data.hasOwnProperty('header')) {
                // error
        ...
        const allControls = () => {
            ...
            const foundControls: CHControl[] = _.cloneDeep(this.Controls);
        ...
        const allEvents = () => {
            // emits to all in namespace
            const foundEvents: CHEvent[] = _.cloneDeep(this.Events);
        ...
        const observedControls = (whichSocket?: string) => {
        ...
        const observedEvents = (whichSocket?: string) => {
        ...
        const unpublishEvent = (data: CHObserve, callback?) => {
            if (!data.hasOwnProperty('header')) {
                // error
        ...
        const listMyControlsDetails = (whichSocket?: string) => {
            if (whichSocket === undefined) {
                whichSocket = socket.id;
            }
        ...
        const availableControls = (whichSocket?: string) => {
            if (whichSocket == null) {
                whichSocket = socket.id;
            }
            ...
            let foundControls: CHControl[] = foundAllControls.concat(foundAllRoomControls);
        ...
        const availableEvents = (whichSocket?: string) => {
            if (whichSocket == null) {
                whichSocket = socket.id;
            }
        ...
        const clearEvents = (data?: CHClearEvents) => {
        ...
        const getSocketIDFromUsername = (username: string) => {
            for (const [_id, values] of this.Users.entries()) {
        ...
        const getRoomnamesFromSocketID = (socketid: string) => {
        ...
        const getUsernameFromSocketID = (socketid?: string) => {
        ...
        const clearControls = (data?: CHClearControls) => {
            if (Array.isArray(data.header)) {
        ...
        const addUser = (_socket) => {
            ...
            if (!this.Users.has(_socket.id)) {
                let tempUsername: string = generateUsername();
                ...
                const _tempUserDetails: CHUserDetails = {
        ...
        const listControls = () => {
            ...
            let _controls: CHControl[] = _.cloneDeep(this.Controls);
        ...
        const listEvents = () => {
            // clone so we are not changing the 'deep' values
            let _events: CHEvent[] = _.cloneDeep(this.Events);
            ...
            const _eventsArrayObject: CHEvents = {
                events: _events
            }
        ...
        // internal utility
        const getAllRoomsArray = (filter?: boolean) => {
        ...
        // internal utility
        const getAllRoomsDetails = (filter?: boolean) => {
        ...
        const myRooms = (whichSocket?: string) => {
        ...
        const myRoomsList = (whichSocket?: string) => {
            if (whichSocket === null) {
                whichSocket = socket.id;
            }
        ...
        const availableRooms = (whichSocket?: string) => {
            if (whichSocket === undefined) {
                whichSocket = socket.id
            }
        ...
        const availableRoomsList = (whichSocket?: string) => {
            if (whichSocket === undefined) {
                whichSocket = socket.id
            }
            ...
            const availableRoomsListObject: CHAllRooms = {
        ...
        const sendError = (message: string) => {
            const _messageObject: CHMessage = {
                message
            }
        ...
        // used
        // utilities
        const lowercaseString = (_string: string) => {
    ...
    modeHandler = (_mode, fallback?: Mode) => {
    ...
    boolHandler = (bool: string | boolean) => {
        if (typeof bool === 'string') {
            const _bool: boolean = (bool.toLowerCase() === 'true') ? true : false;
            return _bool
        } else {
            return bool;
        }
    ...
    isCHControlToServer(object): object is CHControlToServer {
